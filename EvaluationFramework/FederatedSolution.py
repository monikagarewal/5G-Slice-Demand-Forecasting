...
##create test data in a similar fashion to the train dataset
##load dataset
def load_test_datasets(clientId, sliceSet):
	filename = 'client-' + str(clientId) + '.csv'
	clientData = pd.read_csv(os.path.join(clientDirectoryPath, filename), sep=',', header=0, low_memory=False, infer_datetime_format=True)
	clientData = clientData.drop(['Date','StartTime'], axis=1)
...
for model in models:
	...
	for configuration in config_list:
		...
		algoDirPath = baseResultsDir + algorithm
		os.makedirs(algoDirPath, exist_ok=True)
		...
		for obsvWinLen, predWinLen in zip(OBSERVATION_WINDOW_RANGE, PREDICTION_WINDOW_RANGE):
			...
			comboDir = 'Combo-' + str(obsvWinLen) +'-'+str(predWinLen) 
			resultDirectoryPath = algoDirPath + comboDir 
			...
			print('###################### Test metrics - Stop #########################')
			############################# Algorithm-Metrics ####################################
			if algorithm in algo_count:
				num_samples = algo_count[algorithm] + 1
				algo_count[algorithm] = num_samples
				algo_mse_metrics[algorithm] = calculate_new_avg(test_metrics['loss'], algo_mae_metrics[algorithm], num_samples)
				algo_mae_metrics[algorithm] = calculate_new_avg(test_metrics['mean_absolute_error'], algo_mae_metrics[algorithm], num_samples)
			else:
				algo_count[algorithm] = 1
				algo_mse_metrics[algorithm] = test_metrics['loss']
				algo_mae_metrics[algorithm] = test_metrics['mean_absolute_error']
			...
			for clientId in selected_clients:
				clientTestDataset, slice_datasets = load_test_datasets(clientId)
		...
		plot_bargraph_metrics(win_mae_metrics, win_mse_metrics, title, os.path.join(algoDirPath, 'Window-Combination-Metrics.pdf'))
	title = model + '-Algorithm-Average-Metrics' 
	plot_bargraph_metrics(algo_mae_metrics, algo_mse_metrics, title, os.path.join(modelDirPath, 'algorithm-combination-metrics.pdf'))
	sys.stdout.close()
