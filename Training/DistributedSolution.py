...
base_model_dir = '../Models/Mi-2013/Local/'
...
#Create if directory doesn't exist to avoid failure
...
for obsv_win_len, pred_win_len in zip(OBSERVATION_WINDOW_RANGE, PREDICTION_WINDOW_RANGE):
	...
	print('################################## '+ combo +' ##################################')
	#Load all datasets into the collection - why here? - Dataset depends on window length config
	client_datasets = {}
	for client_id in client_ids:
		client_datasets[client_id] = load_client_dataset(client_id)
		print('################################## Loaded Client:'+ str(client_id) +' Data ##################################')
	#Iterate through each client & train their local model
	for client_id in client_ids:
		print('################################## Training at Client:'+ str(client_id) +' ##################################')
		client_dir = combo_dir + 'client-' + str(client_id) + '/' 
		#client_datasets = dic{'client_id':(train_dataset}
		...
		for configuration in config_list:
			...
			algo_dir = client_dir + opalg + '/' + scler + '/'
			...
			for model in models:
				...
				print('################################## Training model ' + model + ' of client:'+ str(client_id) +'##################################')
				#Train a model at each client using the local dataset
				history = keras_model.fit(client_data,epochs=NUM_EPOCHS,verbose=1)
				elapsed_time = time.time() - start_time
				print('Time Elapsed - (Model Creation & Training) -' + str(elapsed_time))
				...
				print('################################## Saved model ' + model + ' of client:'+ str(client_id) +'##################################')
				model_key = combo + '/' + opalg + '/' + scler + '/' + model
				#Calculate the average of training time over all the clients for a configuration
				if model_key in tr_time:
					num_instances = tr_time[model_key][1]
					tr_time[model_key][1] = num_instances + 1
					tr_time[model_key][0] = (tr_time[model_key][0] * num_instances + elapsed_time) / (num_instances + 1)
				else:
					tr_time[model_key] = [elapsed_time,1]
#Helper function to extract config from key : combo/opalg/scler/model 
def extract_config(key, sep='/'):
	config = key.split(sep)
	return config[0],config[1],config[2],config[3]
#Iterate through the per model training time collection and update training time log
for model_key in tr_time:
	tr_time = tr_time[model_key][0]
	combo, opalg, scler, model = extract_config(model_key,'/')
	tr_time_log = tr_time_log.append({'Approach':'Local','Combination':combo,'Optimizer':opalg,'Scaler':scler,'Model':model,'Elapsed-Time':tr_time}, ignore_index=True)
