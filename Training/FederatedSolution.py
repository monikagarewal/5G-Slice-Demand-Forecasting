...
base_model_dir = '../Models/Mi-2013/Federated/'
...
def create_keras_model():
	...
	if scler == 'Quantile':
		...
		act_fn = 'relu' 
	#This resets the global state and saves on memory - models apparently cannot be reset - Not working with TFF
...
def load_client_training_dataset(clientId):
	...
	for sliceId in slice_set:
		...
		slice_dataset = preprocess(slice_TrainData, sliceId, obsv_win_len, pred_win_len)
...
def create_train_dataset_for_client_fn(clientId):
	...
	return load_client_training_dataset(clientId)
...
for obsv_win_len, pred_win_len in zip(OBSERVATION_WINDOW_RANGE, PREDICTION_WINDOW_RANGE):
	...
	for configuration in config_list:
		...
		for model in models:
			...
			#Issue - doesn't work if using same client optimizer object. not able to do clear_session either
			if opalg == 'SGD':
			...
			tr_time_log = tr_time_log.append({'Approach':'Federated','Combination':combo,'Optimizer':opalg,'Scaler':scler,'Model':model,'Elapsed-Time':elapsed_time}, ignore_index=True)
